...
impl CryptoComponent {
    ...
    #[cfg(target_os = "windows")]
    async fn load_master_key_from_dpapi(
        &self,
    ...
    #[cfg(target_os = "windows")]
    async fn save_master_key_with_dpapi(
        &self,
        key_path: &std::path::Path,
        key: &Key<Aes256Gcm>,
    ) -> AppResult<()> {
    ...
    pub fn encrypt(&self, data: &SecretData) -> AppResult<EncryptedData> {
        let master_key = self.master_key.as_ref().ok_or_else(|| {
            AppError::authentication("Master key not initialized", None::<std::io::Error>)
        })?;
        ...
        let ciphertext = cipher
            .encrypt(&nonce, data.expose_secret())
        ...
        log::debug!(
            "Data encrypted successfully ({} bytes -> {} bytes)",
            data.len(),
    ...
    pub fn decrypt(&self, encrypted_data: &EncryptedData) -> AppResult<SecretData> {
        ...
        log::debug!(
            "Data decrypted successfully ({} bytes -> {} bytes)",
            encrypted_data.ciphertext.len(),
            plaintext.len()
        );
    ...
    pub fn decrypt_from_json(&self, json: &str) -> AppResult<SecretData> {
        let encrypted_data: EncryptedData = serde_json::from_str(json).map_err(|e| {
            AppError::serialization("Failed to deserialize encrypted data", Some(e))
        })?;
...
mod tests {
    ...
    async fn test_encryption_roundtrip() {
        ...
        assert_eq!(
            decrypted.expose_secret_string().unwrap(),
            "sensitive test data"
        );
    }
