...
impl ApiComponent {
    ...
    ) -> AppResult<RecordingSearchResponse> {
        // 事前条件の検証
    ...
    pub async fn get_all_recordings(
        &self,
        request: RecordingSearchRequest,
    ) -> AppResult<Vec<MeetingRecording>> {
        ...
        log::info!(
            "Retrieved {} meetings across {} pages",
            all_meetings.len(),
            total_pages
        );
        Ok(all_meetings)
    ...
    pub async fn get_meeting_summary(
        &self,
        meeting_uuid: &str,
    ) -> AppResult<Option<MeetingSummaryResponse>> {
        self.wait_for_rate_limit().await?;
        ...
        let url = format!(
            ...
            self.config.base_url, encoded_uuid
        );
        ...
        if status == StatusCode::FORBIDDEN || status == StatusCode::NOT_FOUND {
            self.record_api_call(duration, true).await;
            log::info!(
                "Meeting summary not available for meeting_uuid={} (status={})",
                meeting_uuid,
                status
            );
            return Ok(None);
        ...
        if status == StatusCode::TOO_MANY_REQUESTS {
            ...
            return Err(AppError::rate_limit_with_retry(
                "Meeting summary API rate limit exceeded",
                retry_after,
            ));
        }
        ...
        if !status.is_success() {
            ...
            let error_body = response.text().await.unwrap_or_default();
            log::warn!(
                "Meeting summary API error for meeting_uuid={}: {} - {}",
                meeting_uuid,
                status,
                error_body
            );
            return Err(AppError::external_service(format!(
                "Meeting summary API error: {} - {}",
                status, error_body
            )));
        }
        ...
        log::debug!(
            "Meeting summary response for meeting_uuid={}: {}",
            meeting_uuid,
        ...
        let summary: MeetingSummaryResponse =
            serde_json::from_str(&response_text).map_err(|e| {
                AppError::data_format("Failed to parse meeting summary response", Some(e))
            })?;
...
impl ComponentLifecycle for ApiComponent {
    ...
    async fn shutdown(&mut self) -> AppResult<()> {
        ...
        log::info!(
            "API call statistics: total={}, success={}, errors={}, rate_limit_errors={}",
            metrics.total_calls,
...
mod tests {
    ...
    fn test_recording_file_type_serialization() {
        // 各バリアントのシリアライズ確認
        assert_eq!(
            serde_json::to_string(&RecordingFileType::MP4).unwrap(),
            "\"MP4\""
        );
        assert_eq!(
            serde_json::to_string(&RecordingFileType::M4A).unwrap(),
            "\"M4A\""
        );
        assert_eq!(
            serde_json::to_string(&RecordingFileType::Transcript).unwrap(),
            "\"TRANSCRIPT\""
        );
        assert_eq!(
            serde_json::to_string(&RecordingFileType::Chat).unwrap(),
            "\"CHAT\""
        );
        assert_eq!(
            serde_json::to_string(&RecordingFileType::ClosedCaption).unwrap(),
            "\"CC\""
        );
        assert_eq!(
            serde_json::to_string(&RecordingFileType::Timeline).unwrap(),
            "\"TIMELINE\""
        );
        assert_eq!(
            serde_json::to_string(&RecordingFileType::Summary).unwrap(),
            "\"SUMMARY\""
        );
    }
    ...
    fn test_recording_file_type_deserialization() {
        // 既知のタイプのデシリアライズ確認
        assert_eq!(
            serde_json::from_str::<RecordingFileType>("\"MP4\"").unwrap(),
            RecordingFileType::MP4
        );
        assert_eq!(
            serde_json::from_str::<RecordingFileType>("\"M4A\"").unwrap(),
            RecordingFileType::M4A
        );
        assert_eq!(
            serde_json::from_str::<RecordingFileType>("\"TRANSCRIPT\"").unwrap(),
            RecordingFileType::Transcript
        );
        assert_eq!(
            serde_json::from_str::<RecordingFileType>("\"CHAT\"").unwrap(),
            RecordingFileType::Chat
        );
        assert_eq!(
            serde_json::from_str::<RecordingFileType>("\"CC\"").unwrap(),
            RecordingFileType::ClosedCaption
        );
        assert_eq!(
            serde_json::from_str::<RecordingFileType>("\"TIMELINE\"").unwrap(),
            RecordingFileType::Timeline
        );
        assert_eq!(
            serde_json::from_str::<RecordingFileType>("\"SUMMARY\"").unwrap(),
            RecordingFileType::Summary
        );

        // 未知のタイプはUnknownにフォールバック
        assert_eq!(
            serde_json::from_str::<RecordingFileType>("\"UNKNOWN_TYPE\"").unwrap(),
