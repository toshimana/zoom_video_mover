...
impl AuthToken {
    ...
    pub fn has_scope(&self, required_scope: &str) -> bool {
        assert!(
            ...
            "required_scope must not be empty"
        );
...
impl AuthComponent {
    ...
    pub fn generate_auth_url(&mut self) -> AppResult<(String, String)> {
        let oauth_client = self.oauth_client.as_ref().ok_or_else(|| {
            AppError::authentication("OAuth client not initialized", None::<std::io::Error>)
        })?;
    ...
    pub async fn exchange_code_for_token(
        ...
        let flow_state = self.pending_flows.remove(state_id).ok_or_else(|| {
            AppError::authentication("Invalid or expired state", None::<std::io::Error>)
        })?;
    ...
    pub async fn refresh_token(&mut self) -> AppResult<AuthToken> {
        ...
        let current_token = self.current_token.as_ref().ok_or_else(|| {
            AppError::authentication("No current token available", None::<std::io::Error>)
        })?;

        let refresh_token = current_token.refresh_token.as_ref().ok_or_else(|| {
            AppError::authentication("No refresh token available", None::<std::io::Error>)
        })?;
    ...
    pub async fn save_token_securely(&self) -> AppResult<()> {
        ...
        if !self.crypto.is_initialized() {
            return Err(AppError::authentication(
                "Crypto component not initialized",
                None::<std::io::Error>,
            ));
        }
