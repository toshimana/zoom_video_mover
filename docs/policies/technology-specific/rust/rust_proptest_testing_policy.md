# Rust・Proptest・Cargoテスト方針 - Zoom Video Mover

**技術要素**: Rust, proptest, cargo, tokio-test  
**依存関係**: 
- Rust 1.70+ (テストフレームワーク)
- proptest 1.0+ (Property-basedテスト)
- cargo (ビルド・テストツール)
- tokio-test (非同期テスト)

## Rust・Proptest技術固有のテスト方針
...
fn benchmark_download_operations(c: &mut Criterion) {
    ...
    group.measurement_time(Duration::from_secs(30)); // 十分な測定時間
