# Rust・tokio・eGUI設計方針 - Zoom Video Mover

**技術要素**: Rust, tokio, egui/eframe  
**依存関係**: 
- Rust 1.70+ (型安全性・メモリ安全性)
- tokio 1.0+ (非同期ランタイム)
- egui 0.22+ / eframe (GUI フレームワーク)
- reqwest (HTTP クライアント)

## Rust・tokio・eGUI技術固有の設計方針
...
この設計方針に従うことで、**ユーザーのニーズを満たし、技術的に優秀で、持続的に成長可能**なソフトウェアシステムを構築できます。