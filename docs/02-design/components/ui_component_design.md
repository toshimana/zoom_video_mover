...
### 責任・役割
- **即時モードGUI制御**: egui/eframe即時モードパターンによる毎フレーム再構築型UI
- **宣言的状態管理**: アプリケーション状態に基づく宣言的UI描画・状態同期
- **リアルタイム進捗表示**: 即時レスポンシブ進捗バー・通知・ステータス表示
...
/// 
/// # 事後条件
/// - UI全体が現在状態に基づいて再描画される
/// - イベントが適切に処理される
/// 
/// # 不変条件
/// - フレーム描画中にUI状態の一貫性が保たれる
impl eframe::App for ZoomVideoMoverApp {
    fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        // 毎フレーム：状態に基づくUI再構築
        self.render_immediate_ui(ctx, frame);
        
        // 継続描画が必要な場合のみリクエスト
        if self.requires_continuous_repaint() {
            ctx.request_repaint();
        }
    }
}
...
#### 宣言的UI記述パターン
- **状態ベース描画**: 現在の状態のみに基づいてUI要素を生成
- **条件分岐による表示制御**: 状態に応じた動的UI要素表示
- **イベント即座反映**: UI操作の即座状態反映・再描画

### eGUI特有設計考慮事項

#### レスポンシブ設計
- **動的レイアウト**: ウィンドウサイズ変更の即座対応
- **スケーラブルUI**: DPI変更・画面解像度変更対応
- **アニメーション**: スムーズな状態遷移表現
