...
### 構文チェック環境
- **ツール**: `3rdparty/plantuml.jar`
- **必須**: Javaランタイム（JRE/JDK）
...
### PlantUMLファイル作成時の注意点
1. **構文チェック必須**: 新規作成・修正時は必ず構文チェックを実行
2. **一般的なエラー**:
   - `par`ブロック内では`and`ではなく`else`を使用
   - Rustの`trait`は`interface`として定義
   - `rectangle`要素内にノードを含める場合の制約に注意
3. **エラー時の対処**: エラーメッセージが不明確な場合は`-v`オプションで詳細確認
